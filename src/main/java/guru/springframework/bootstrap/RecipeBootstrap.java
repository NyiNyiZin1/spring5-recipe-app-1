...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public RecipeBootstrap(RecipeRepository recipeRepository,
                           ...
                           UnitOfMasureRepository unitOfMasureRepository) {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        log.debug("Looding bootstrap Data");
    }

    private List<Recipe> getRecipes() {
        log.debug("It is in RecipeBootstrap.getRecipes..");
        List<Recipe> recipes = new ArrayList<>(2);

        //get UOMs
        Optional<UnitOfMeasure> eachUomOptional = unitOfMasureRepository.findByDescription("Each");
        if (!eachUomOptional.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> tablespoonUomOptional = unitOfMasureRepository.findByDescription("Tablespoon");
        if (!tablespoonUomOptional.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> teaspoonUomOptional = unitOfMasureRepository.findByDescription("Teaspoon");
        if (!teaspoonUomOptional.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> dashUomOptional = unitOfMasureRepository.findByDescription("Dash");
        if (!dashUomOptional.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> pintUomOptional = unitOfMasureRepository.findByDescription("Pint");
        if (!pintUomOptional.isPresent()) {
            throw new RuntimeException("Expected UOM Not Found");
        }

        Optional<UnitOfMeasure> cupsUomOptional = unitOfMasureRepository.findByDescription("Cup");
        ...
        // very redundant add helper method, and make this simpler
        guacRecipe.getIngredients().add(new Ingredient("reipe avocados", new BigDecimal(2), eachUom));
        ...
        tacosRecipe.getIngredients().add(new Ingredient("packed baby arugula (3 ounces)", new BigDecimal(3), cupsUom));
        ...
        tacosRecipe.getIngredients().add(new Ingredient("cut into wedges", new BigDecimal(1), eachUom));
